...
- [ ] **Split `deps` so Go-only gates don't provision Node/pnpm/Newman**: `static-check`'s prerequisites (`format-check`, `lint-ci`, `lint`, `sec`, `vulncheck`, `secrets`, `release-check`) each depend on `deps`, which installs Node + corepack + pnpm + Newman. `Cache Newman` exists **only** in the `e2e` job, so `test/node_modules` never exists in `static-check` and `pnpm install` runs on every single run. This is *why* the 2026-07-20 corepack breakage took down linting rather than just e2e. Fix: add a `deps-go: deps-mise` tier mirroring the existing `deps-image`. Enumerate targets **from the file** (`grep -nE '^[a-z-]+:.*\bdeps\b' Makefile` → **31** lines, not the ~14 you'd list from memory); `ci:` names `deps` directly so it must be repointed too; `renovate-validate` genuinely needs Node (`npx`) and must stay on full `deps`, as must `e2e` / `e2e-quick`. Benefit: `static-check` stops installing Newman on every run, and a Newman-provisioning bug can no longer redden linting.
- [ ] **Gate that the Go-only closure never provisions Newman** (depends on the split above): assert `make -n static-check` emits no `corepack` and no `newman` token. **Do NOT put `pnpm` in the token set** — `Makefile:503` (`trivy-fs`, a `static-check` prereq) contains `--skip-dirs test/node_modules,.pnpm-store`, so a `pnpm` grep is red on a *correctly fixed* tree and the gate would be deleted on day one. RED-prove it by repointing one Go-only gate back at `deps` and confirming it fails.
